...
Double_t theta1 = 21.;           //Scattering angle of incident electron degrees.
  ...
  //Using Nilanga
  T->Draw("TMath::ACos(  (cos(21.*3.14159265359/180.)-L.gold.ph*sin(21.*3.14159265359/180.))/pow(1+pow(L.gold.th,2.)+pow(L.gold.ph,2.),0.5)  )*180./3.14159265359:3.05666452*(1+L.gold.dp)>>h2(1000,2.7,3.4,1000,17.,24.)","L.cer.asum_c>60 && (L.prl1.e+L.prl2.e)/L.tr.p/1000>0.7 && L.tr.tg_y>-0.028 && L.tr.tg_y<0.028 && L.tr.n==1 && (DBB.evtypebits>>3)&1 && EKL.x_bj>2.98 && EKL.x_bj<3.11");

  h2->SetTitle("Scattering Angle as a Function of E' (Elastic Band)");
  h2->GetXaxis()->SetTitle("E' (GeV)");
  h2->GetYaxis()->SetTitle("Scattering Angle (deg.)");
  gStyle->SetOptStat(0);
  h2->Draw();
  ...
  //T->Draw("EKL.x_bj>>h2(1000,0.,4.)","L.tr.tg_y>-0.028 && L.tr.tg_y<0.028 && L.tr.n==1 && (DBB.evtypebits>>3)&1");
  T->Draw("EKL.x_bj>>h3(1000,0.,4.)","L.tr.tg_y>-0.028 && L.tr.tg_y<0.028 && L.tr.n==1 && (DBB.evtypebits&1<<3)==1<<3");
  h3->GetXaxis()->SetTitle("X_{Bj}");

  Int_t nevts = T->GetEntries();
  cout<<"# Events = "<<nevts<<endl;

  //Create an exponential fit function.
  Bool_t reject=1;
  //reject = kTRUE;
  ...
  TF1 *func = new TF1("fitf",fitf,2.,3.5,2);
  h3->Fit("fitf","r0");
  func->SetParameter(0,14.);
