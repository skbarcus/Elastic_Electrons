...
Double_t M = 3.0160293*0.9315;         //Mass of Helium 3 GeV.
...
Double_t dE = 0.151;              //Momentum acceptance of HRS. +- 4.5%
Double_t dtheta = 0.03*1/deg2rad;    //Horizontal angular acceptance of HRS. +- 30 mrad.

Double_t xmin = 2.8;
Double_t xmax = 3.3;//E+0.1;
...
void E_Prime_vs_Theta_Elastic() 
{
  ...
  fEprime->GetHistogram()->SetTitle("^{3}He Elastic Scattering Band and Spectrometer Acceptance for KIN 3.2");
  fEprime->GetHistogram()->GetYaxis()->SetTitle("Theta (#circ)");
  fEprime->GetHistogram()->GetXaxis()->CenterTitle(true);
  fEprime->GetHistogram()->GetXaxis()->SetTitle("E' (GeV)");
  fEprime->GetHistogram()->GetYaxis()->CenterTitle(true);
  ...
  Double_t ypadmin = gPad->GetUymin();
  Double_t xpadmin = gPad->GetUxmin();
 
  //Line representing the lower momentum acceptance.
  TLine *line1 = new TLine(Ep-dE, ypadmin, Ep-dE, ypadmax);
  line1->SetLineColor(kBlack);
  line1->SetLineWidth(2);
  line1->Draw("SAME");
  
  //Line representing the upper momentum acceptance.
  TLine *line2 = new TLine(Ep+dE, ypadmin, Ep+dE, ypadmax);
  line2->SetLineColor(kBlack);
  line2->SetLineWidth(2);
  line2->Draw("SAME");

  //Line representing the lower angular acceptance.
  TLine *line3 = new TLine(xpadmin, theta-dtheta, xpadmax, theta-dtheta);
  line3->SetLineColor(kBlack);
  line3->SetLineWidth(2);
  line3->Draw("SAME");
  
  //Line representing the upper angular acceptance.
  TLine *line4 = new TLine(xpadmin, theta+dtheta, xpadmax, theta+dtheta);
